...
feature_names = ['length', '%max_ascii_skip_0', '%max_digit_skip_0',
                 '%asciis', '%digits', '%punctuations', '%others', '%name_keywords', '%address_keywords', '%phone_keywords',
                'count_vnese_characters', '%vnese_characters']
...
def feature(text):
    ...
    return [
             ...
             sum([1 for c in text if (c in unicodeCharacterSet)]),
             sum([1 for c in text if (c in unicodeCharacterSet)]) / textLen
            ]
