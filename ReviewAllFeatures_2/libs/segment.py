...
def checkTemplate(dct, dft):
    if ('#digit' in dft):
        if (dct['phone']['features'][dft['#digit']] < 7):
            return False

    if ('first_character_digit' in dft):
        if (dct['phone']['features'][dft['first_character_digit']] != 1):
            return False

    if ('last_character_digit' in dft):
        if (dct['phone']['features'][dft['last_character_digit']] != 1):
            return False

    if ('%kwAddress' in dft and '%kwPhone' in dft):
        if (dct['phone']['features'][dft['%kwAddress']] > 0 and dct['phone']['features'][dft['%kwPhone']] == 0):
            return False

    # if ('%kwPhone' in dft and '#ascii' in dft):
    #     if (dct['phone']['features'][dft['%kwPhone']] == 0 and dct['phone']['features'][dft['#ascii']] > 0):
    #         return False

    if ('#ascii' in dft):
        if (dct['name']['features'][dft['#ascii']] < 7):
            return False

    if ('first_character_digit' in dft):
        if (dct['name']['features'][dft['first_character_digit']] == 1):
            return False
