...
def loadDictionary(file=folder_dictionary + '/' + files_dictionary):
    tmp = loadJson(file)
    return tmp
...
def saveResults(titles, input_tuple, templateList, folder=folder_results, file=file_segment_address_result):
    ...
    for i, templates in zip(range(len(templateList)), templateList):
        for j, template in zip(range(len(templates)), templates):
            t = [i, j, template['name']['term'], template['address']['term'], template['phone']['term'],
                 ...
                 template['score'],
                 template['name']['preprocessed'], template['address']['preprocessed'], template['phone']['preprocessed']] \
                 + template['phone']['features']
            sg_1.append(t)
