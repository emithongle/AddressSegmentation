...
def parseAddress(textList, func=None, pfunc=None):
    ...
    for (i, text) in zip(range(len(textList)), textList):
        ...
        templates = templateFiler(clf_model, ptemplates, func, pfunc)
...
def templateFiler(clf, ptemplates, func, pfunc):
    templates = []
    ...
    for terms in ptemplates:
        X = np.asarray([fe.extractFeatureText(func, pfunc, term) for term in terms])
        cls = clf.predict(X)
        ...
        if (tmp == list(range(len(terms)))):
            ...
            # Recheck template
            pctdigits = sum([1 for i in dct['phone']['term']]) / len(dct['phone']['term'])
            if (len(dct['phone']['term']) >= 7 and pctdigits > 0.8):
                templates.append(dct)
